  ...
  async readAll(where) {
    if (where) {
      const [rows] = await this.database.query(
        `select boat.id, boat.coord_x, boat.coord_y, boat.name, tile.type, tile.has_treasure from ${this.table} join tile on boat.coord_x = tile.coord_x and boat.coord_y = tile.coord_y where boat.name = ? `,
        [where.name]
      );
      return rows;
    }
